...
$(document).ready(function() {
    ...
    @if ($errors->any())
        ...
        // Logic to open the correct modal if validation fails
        @if ($errors->has('update_name_skill')) // Assuming update name input is 'update_name_skill'
            $('#updateSkillModal').modal('show');
        @endif
        @if ($errors->has('name') && !isset($errors->messages()['update_name_skill'])) 
            $('#storeSkillModal').modal('show');
        @endif
    @endif
    ...
    function fetchData(page = 1, search = '', per_page = 10) {
        ...
        $.ajax({
            url: '{{ route('skills.data') }}', // Route change
            method: 'GET',
            ...
            success: function(response) {
                ...
                if (response.data.length > 0) {
                    ...
                    html = '<tr><td colspan="4" class="text-center">No skills found.</td></tr>';
                }
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        const deleteUrl = '{{ route('skills.destroy', ['skill' => ':id']) }}'.replace(':id', skillId); // Route change
    ...
    $(document).on('click', '.edit-btn', function() {
        ...
        $('#update_skill_id').val(id); // ID change
        $('#update_name_skill').val(name); // ID change
        
        // Update the form action URL using the resource update route
        const updateUrl = '{{ route('skills.update', ['skill' => ':id']) }}'.replace(':id', id); // Route change
        $('#updateSkillForm').attr('action', updateUrl);
        
        // Clear previous validation errors if any were shown
        $('#updateSkillForm').find('.is-invalid').removeClass('is-invalid');
        $('#updateSkillForm').find('.invalid-feedback').remove();
    });
